...
#if defined(NDEBUG)
# error "Podcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfb9cfd6f2cfc5cd462bf3c977c6abfb097c23af8b2b055085a8d350fd22fb087");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Podcoin: starting difficulty is 1 / 2^12
...
{
    // Podcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Podcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Podcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Podcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Podcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Podcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4d787a127aab9a8e9bf33e95dd8c319a508be65410d73e2e66f82503460982fc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Caliente iTechs POD Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e772bbc53da255003508107cb18078be8a0b5f3c3f8748828b507c1b696328b84dd71956b36a723003ee280c4d0fa5d4ee8dcfa0dbec9231c9be1156a82619c5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517043198;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085387617;

        if (fTestNet)
        {
            block.nTime    = 1517043185;
            block.nNonce   = 390171377;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x2130e9897ea472f5b711a75052a3ec376e4feb6c3fd6e18b1c66d9bb30160548"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xb1, 0xde }; // Podcoin: increase each by adding 2 to bitcoin's value.
...
void static PodcoinMiner(CWallet *pwallet)
{
    printf("PodcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("podcoin-miner");
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PodcoinMiner, pwallet));
}
